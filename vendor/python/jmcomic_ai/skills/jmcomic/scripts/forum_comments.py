#!/usr/bin/env python3
"""Fetch one page of site-wide JMComic comments."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fetch site-wide JMComic comments")
...
def main():
    ...
    try:
        ...
        result = service.get_forum_comments(page=args.page)
    except Exception as e:
        print(f"Error: failed to fetch forum comments on page {args.page}: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output:
        ...
        try:
            ...
            print(f"Error: failed to export forum comments to {output_path}: {e}", file=sys.stderr)
            sys.exit(1)
        print(f"Exported forum comments to {resolved_output_path}")
    else:
        print(output_text)
